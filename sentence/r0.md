...
To address this problem, we propose to simply upper-bound the less biased value estimate Qθ2 by the biased estimate Qθ1.

이 문제를 해결하기 위해, 우리는 편향된 추정치 Qθ1에 의해 덜 편향된 가치 추정치 Qθ2를 단순히 상한으로 설정하는 것을 제안합니다.
...
$$y_1 = r + \gamma \underset{i=1, 2}{\min}Q_{\theta'_{i}}(s', \pi_{\phi_{1}}(s')). \tag{10}$$
